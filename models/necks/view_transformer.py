...
class LSSViewTransformer(BaseModule):
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ...
        bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                               ...
                               interval_lengths) # (B, C, Z, Y, X)
        # collapse Z
        if self.collapse_z:
            bev_feat = torch.cat(bev_feat.unbind(dim=2), 1) # (B, Z*C, Y, X)
            bev_feat = bev_feat.permute(0, 1, 3, 2) # (B, Z*C, X, Y)
        else:
             bev_feat = bev_feat.permute(0, 1, 4, 3, 2) # (B, C, X, Y, Z)
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        if self.accelerate:
            ...
            bev_feat = bev_pool_v2(depth, feat, self.ranks_depth,
                                   ...
                                   self.interval_lengths) # (B, C, Z, Y, X)
            ...
            bev_feat = self.voxel_pooling_v2(
                ...
                tran_feat.view(B, N, self.out_channels, H, W)) # (B, C, X, Y, Z)
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def forward(self, input, stereo_metas=None):
        ...
        return bev_feat, depth # (B, C, X, Y, Z)
