...
# modified from https://github.com/facebookresearch/Mask2Former/blob/main/mask2former/modeling/matcher.py#L70
class HungarianMatcherMix(BaseModule):
    """This class computes an assignment between the targets and the predictions of the network
    ...
    def forward(self, mask_pred, class_pred, mask_gt, class_gt, mask_camera):
        ...
        """
        if self.stuff_flag:
            indices = self.stuff_forward(mask_pred, class_pred, mask_gt, class_gt, mask_camera)
        else:
            indices = self.thing_forward(mask_pred, class_pred, mask_gt, class_gt, mask_camera)

        return indices

    @torch.no_grad()
    def thing_forward(self, mask_pred, class_pred, mask_gt, class_gt, mask_camera):
        num_queries = class_pred.shape[0]

        num_instances = class_gt.shape[0]  # must be here, cause num of instances may change after masking
        ...
        # Compute the classification cost. We use focal loss provided by mmdet as sparsebev does
        cost_class = self.loss_focal(class_pred, class_gt.long())

        # all masks share the same set of points for efficient matching!
        tgt_mask = mask_gt.view(mask_gt.shape[0], -1)
        out_mask = mask_pred.view(mask_pred.shape[0], -1)

        with autocast(enabled=False):
            out_mask = out_mask.float()
            tgt_mask = tgt_mask.float()
            # Compute the focal loss between masks
            cost_mask = batch_sigmoid_ce_loss(out_mask, tgt_mask, mask_camera)

            # Compute the dice loss betwen masks
            cost_dice = batch_dice_loss(out_mask, tgt_mask, mask_camera)
        ...
        C = C.reshape(num_queries, -1).cpu()
        C[C.isnan()] = 1e6 # to fix "ValueError: matrix contains invalid numeric entries"
        src_idx, tgt_idx = linear_sum_assignment(C)
            
        return (torch.as_tensor(src_idx, dtype=torch.int64), 
                torch.as_tensor(tgt_idx, dtype=torch.int64))
    
    @torch.no_grad()        
    def stuff_forward(self, mask_pred, class_pred, mask_gt, class_gt, mask_camera):
        bs, num_queries = class_pred.shape[:2]
