...
def random_drop_tracks(active_track_instances: Instances, drop_probability: float) -> Instances:
    if drop_probability > 0 and len(active_track_instances) > 0:
        keep_idxes = torch.rand_like(active_track_instances.scores) > drop_probability
        remain_track_instances = active_track_instances[keep_idxes]
    if len(remain_track_instances) == 0:
        return active_track_instances
    return remain_track_instances
...
class QueryInteractionModule(QueryInteractionBase):
    ...
    def forward(self, data) -> Instances:
        ...
        active_track_instances = self._update_track_embedding(active_track_instances) # self attention to update active_track_instances.query, linear to update query_pos
