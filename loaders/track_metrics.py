...
# This class is borrowing from 4D-PLS (https://github.com/MehmetAygun/4D-PLS/blob/master/utils/eval_np.py)
class PanopticEval:
...
class Panoptic4DEval:
  ...
  def getSemIoU(self):
    ...
    num_present_classes = len(self.include)
  ...
  def addBatchPanoptic4D(self, seq, x_sem_row, x_inst_row, y_sem_row, y_inst_row):
    ...
    # zero is empty(no instance), so we don't need to plus 1 to instance_x/y
    # there are filter y_inst_in_cl[y_inst_in_cl > 0] to remove empty.
    x_inst_row = x_inst_row
    y_inst_row = y_inst_row
  ...
  def getPQ4D(self):
    ...
    AQ_overall = np.sum(self.pan_aq_ovr)/ np.sum(num_tubes)
...
class OccPanoptic4DEval(Panoptic4DEval):
  def __init__(self, n_classes, device=None, ignore=None, offset=2 ** 32, min_points=2):
    self.n_classes = n_classes
    assert (device == None)
    self.ignore = np.array(ignore, dtype=np.int64)
    self.include = np.array([n for n in range(self.n_classes) if n not in self.ignore], dtype=np.int64)
  ...
  def addBatch(self, seq, x_sem, x_inst, y_sem, y_inst, interested_mask):  # x=preds, y=targets
    ''' IMPORTANT: Inputs must be batched. Either [N,H,W], or [N, P]
    '''
    x_sem = x_sem[interested_mask]
    x_inst = x_inst[interested_mask]
    y_sem = y_sem[interested_mask]
    y_inst = y_inst[interested_mask]
    # add to IoU calculation (for checking purposes)
    self.addBatchSemIoU(x_sem, y_sem)
  ...
  def getPQ4D(self):
    ...
    out_dict['PQ4D'] = PQ4D
    out_dict['AQ_overall'] = AQ_overall
    out_dict['AQ'] = AQ
    out_dict['AQ_p'] = AQ_p
    out_dict['AQ_r'] = AQ_r
    out_dict['iou'] = iou
    out_dict['iou_mean'] = iou_mean
    out_dict['iou_p'] = iou_p
    out_dict['iou_r'] = iou_r
    print(out_dict)

    return PQ4D, AQ_overall, iou_mean

import pandas as pd

class OccPanoptic4DEvalDetail(Panoptic4DEval):

  def __init__(self, **kwargs):
    super(OccPanoptic4DEvalDetail, self).__init__(**kwargs)
    self.df = pd.DataFrame(columns=['seq', 'inst_number', 'PQ4D', 'AQ_overall', 'AQ', 'AQ_p', 'AQ_r', 'iou', 'iou_mean', 'iou_p', 'iou_r'])


  def addBatch(self, seq, x_sem, x_inst, y_sem, y_inst, interested_mask):  # x=preds, y=targets
    ''' IMPORTANT: Inputs must be batched. Either [N,H,W], or [N, P]
    '''
    px_iou_conf_matrix = np.zeros((self.n_classes, self.n_classes), dtype=np.int64)
    ...
    # seq 
    px_iou_conf_matrix = self.addSeqBatchSemIoU(x_sem, y_sem, px_iou_conf_matrix)
    PQ4D, AQ_overall, iou_mean, out_dict = self.getSeqPQ4D(seq, px_iou_conf_matrix)

    # get the unique value of y_inst
    unique_y_inst = np.unique(y_inst)
    len_unique_y_inst = len(unique_y_inst) - 1 # remove 0

    temp_df = pd.DataFrame({
        'seq': [seq],
        'inst_number': [len_unique_y_inst],
        'PQ4D': [out_dict['PQ4D']],
        'AQ_overall': [out_dict['AQ_overall']],
        'AQ': [out_dict['AQ'].tolist()],
        'AQ_p': [out_dict['AQ_p']],
        'AQ_r': [out_dict['AQ_r']],
        'iou': [out_dict['iou'].tolist()],
        'iou_mean': [out_dict['iou_mean']],
        'iou_p': [out_dict['iou_p']],
        'iou_r': [out_dict['iou_r']]
    })

    self.df = pd.concat([self.df, temp_df], ignore_index=True)

  def save_statistic(self, statistic_file):
    if statistic_file is not None:
      self.df.to_csv(statistic_file, index=False)
    else:
      statistic_file = 'statistic.csv'
      self.df.to_csv(statistic_file, index=False)

  def addSeqBatchSemIoU(self, x_sem, y_sem, px_iou_conf_matrix):
    # idxs are labels and predictions
    idxs = np.stack([x_sem, y_sem], axis=0)

    # make confusion matrix (cols = gt, rows = pred)
    np.add.at(px_iou_conf_matrix, tuple(idxs), 1)

    return px_iou_conf_matrix

  def getSeqSemIoU(self, px_iou_conf_matrix):
    tp, fp, fn = self.getSeqSemIoUStats(px_iou_conf_matrix)
    # print(f"tp={tp}")
    # print(f"fp={fp}")
    # print(f"fn={fn}")
    intersection = tp
  ...
  def getSeqSemIoUStats(self, px_iou_conf_matrix):
    # clone to avoid modifying the real deal
    conf = px_iou_conf_matrix.copy().astype(np.double)
    # remove fp from confusion on the ignore classes predictions
    # points that were predicted of another class, but were ignore
    # (corresponds to zeroing the cols of those classes, since the predictions
    # go on the rows)
  ...
  def getSeqPQ4D(self, seq, px_iou_conf_matrix):
    precs = []
    recalls = []
        ...
        for pr_id, pr_size in cl_preds.items():
          ...
          if TPA_key in cl_intersects:
            ...
            if Prec > 1.0 or Recall >1.0:
              print ('something wrong !!')
        outer_sum_iou += 1.0 / float(gt_size) * float(inner_sum_iou)
      pan_aq[cl] += outer_sum_iou # 1.0/float(len(cl_gts)) # Normalize by #tubes
    ...
    # AQ_overall = np.sum(self.pan_aq_ovr)/ np.sum(num_tubes[1:9])
    AQ_overall = np.sum(pan_aq_ovr)/ np.sum(num_tubes)

    AQ = pan_aq / np.maximum(num_tubes, self.eps)

    iou_mean, iou, iou_p, iou_r = self.getSeqSemIoU(px_iou_conf_matrix)
    ...
    return PQ4D, AQ_overall, iou_mean, out_dict
